...
static void     gtk_im_context_xim_class_fini         (GtkIMContextOXIMClass  *class);
//static void     gtk_im_context_xim_init               (GtkIMContextOXIM       *im_context_xim);
static void     gtk_im_context_xim_init               (GObject               *obj);
...
static GType gtk_type_im_context_oxim = 0;
...
//    (GClassFinalizeFunc) gtk_im_context_xim_class_fini,
    NULL,           /* class_finalize */
    NULL,           /* class_data */
...
GType
gtk_oxim_context_get_type (void)
{
    //IDEBUG ("%s", __FUNCTION__);

    if (gtk_type_im_context_oxim == 0) {
	gtk_im_context_xim_register_type (NULL);
    }

    g_assert (gtk_type_im_context_oxim != 0);
    return gtk_type_im_context_oxim;
}
...
//im_context_class->set_surrounding = NULL;
//im_context_class->get_surrounding = NULL;
//
...
static void
gtk_im_context_xim_class_fini (GtkIMContextOXIMClass *class)
{
return;
/*    if (_key_snooper_id != 0) {
...
gtk_im_context_xim_init (GObject *obj)
{
  GtkIMContextOXIM *im_context_xim = GTK_IM_CONTEXT_OXIM (obj);

  im_context_xim->client_window = NULL;
...
//g_debug("[%d]%s\n", __LINE__, __FILE__);
  return GTK_IM_CONTEXT_OXIM (result);
}
...
/*
  ...
  {
	GdkScreen *screen = gdk_window_get_screen(context_xim->client_window);
	GdkDisplay *display = gdk_screen_get_display (screen);
	Display *Xdisplay = GDK_DISPLAY_XDISPLAY (display);
	Window XWin = GDK_WINDOW_XID (context_xim->client_window);
